...
func FuzzLifecycleRuleMatchesUpload(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, prefix, key, filterTagKey, filterTagVal, uploadTagKey, uploadTagVal string) {
		rule := LifecycleRule{
...
func FuzzEvaluateBucketPolicy(f *testing.F) {
	f.Add(`{"Version":"2012-10-17","Statement":[{"Effect":"Allow","Principal":"*","Action":"s3:GetObject","Resource":"arn:aws:s3:::bucket/*"}]}`, "AKID", "s3:GetObject", "arn:aws:s3:::bucket/key")
	f.Add(`{"Version":"2012-10-17","Statement":[{"Effect":"Deny","Principal":"*","Action":"s3:*","Resource":"*"}]}`, "AKID", "s3:PutObject", "arn:aws:s3:::bucket/key")
	f.Add(``, "AKID", "s3:GetObject", "arn:aws:s3:::bucket/key")
	f.Add(`invalid json`, "", "", "")
	f.Add(`{"Version":"2012-10-17","Statement":[]}`, "AKID", "s3:GetObject", "arn:aws:s3:::bucket/key")
