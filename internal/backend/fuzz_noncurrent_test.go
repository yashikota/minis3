...
func FuzzDueNoncurrentTransitionStorageClass(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, sc string, noncurrentDays, newerVersions int, lastModUnix, nowUnix int64, seenMatching int) {
		...
		_, _, _ = dueNoncurrentTransitionStorageClass(transitions, lastMod, now, 24*time.Hour, seenMatching)
	})
}
...
func FuzzApplyNoncurrentExpirationRules(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, key string, noncurrentDays, newerVersions int, lastModUnix, nowUnix int64) {
		...
		versions := []*Object{
			...
			{IsLatest: false, LastModified: lastMod.Add(-24 * time.Hour), IsDeleteMarker: false},
		}
		now := time.Unix(nowUnix, 0)
		_ = applyNoncurrentExpirationRules(key, versions, []LifecycleRule{rule}, now, 24*time.Hour)
	})
}
