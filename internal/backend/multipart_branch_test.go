...
func TestCompleteMultipartUploadBranches(t *testing.T) {
	...
	uploadBucketGone, err := b.CreateMultipartUpload(
		"complete-branch-bucket",
		"obj-bucket-gone",
		CreateMultipartUploadOptions{},
	)
	...
	smallUpload, err := b.CreateMultipartUpload(
		"complete-branch-bucket",
		"small",
		CreateMultipartUploadOptions{},
	)
	...
	invalidETagUpload, err := b.CreateMultipartUpload(
		"complete-branch-bucket",
		"invalid-etag",
		CreateMultipartUploadOptions{},
	)
...
func TestCompleteMultipartUploadDefaultsAndLocks(t *testing.T) {
	...
	if obj.ServerSideEncryption != SSEAlgorithmAWSKMS || obj.SSEKMSKeyId != "kms-default" {
		t.Fatalf(
			"expected default bucket encryption applied, got sse=%q kms=%q",
	...
	objV, err := b.CompleteMultipartUpload(
		"complete-defaults",
		"obj-v",
		uploadV.UploadId,
		[]CompletePart{{PartNumber: 1, ETag: partV.ETag}},
	)
	if err != nil {
...
func TestMultipartListingAndCopyPartBranches(t *testing.T) {
	...
	u1, _ := b.CreateMultipartUpload(
		"list-multipart-branches",
		"a/dir/file-1",
		CreateMultipartUploadOptions{},
	)
	u2, _ := b.CreateMultipartUpload(
		"list-multipart-branches",
		"a/dir/file-2",
		CreateMultipartUploadOptions{},
	)
	u3, _ := b.CreateMultipartUpload(
		"list-multipart-branches",
		"b/key",
		CreateMultipartUploadOptions{},
	)
	...
	partsRes, _, err := b.ListParts(
		"list-multipart-branches",
		"a/dir/file-2",
		u2.UploadId,
		ListPartsOptions{MaxParts: 0},
	)
	if err != nil {
...
func TestListMultipartUploadsSameKeyMarkersAndSort(t *testing.T) {
	...
	if len(res.Uploads) != 0 {
		t.Fatalf(
			"expected no uploads after key marker without upload-id marker, got %+v",
			res.Uploads,
		)
	}
}
