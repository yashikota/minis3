...
func FuzzVerifyPresignedURLV2(f *testing.F) {
	...
	f.Fuzz(func(t *testing.T, accessKeyID, expires, signature, path string) {
		req, err := http.NewRequest(http.MethodGet, path+"?AWSAccessKeyId="+accessKeyID+"&Expires="+expires+"&Signature="+signature, nil)
		if err != nil {
			return
		}
...
func FuzzVerifyPresignedURLV4(f *testing.F) {
	f.Add("AKIAIOSFODNN7EXAMPLE/20230101/us-east-1/s3/aws4_request", "20230101T120000Z", "86400", "host", "abcdef1234567890", "/bucket/key")
	f.Add("", "", "", "", "", "/")
	...
	f.Fuzz(func(t *testing.T, credential, amzDate, expires, signedHeaders, signature, path string) {
		req, err := http.NewRequest(http.MethodGet, path+"?X-Amz-Credential="+credential+"&X-Amz-Date="+amzDate+"&X-Amz-Expires="+expires+"&X-Amz-SignedHeaders="+signedHeaders+"&X-Amz-Signature="+signature, nil)
		if err != nil {
			return
		}
		req.Host = "localhost"
...
func FuzzVerifyAuthorizationHeaderV4(f *testing.F) {
	f.Add("AWS4-HMAC-SHA256 Credential=AKIAIOSFODNN7EXAMPLE/20130524/us-east-1/s3/aws4_request, SignedHeaders=host;x-amz-content-sha256;x-amz-date, Signature=abcdef", "wJalrXUtnFEMI/K7MDENG/bPxRfiCYEXAMPLEKEY")
	f.Add("AWS4-HMAC-SHA256 Credential=AKID/20230101/eu-west-1/s3/aws4_request, SignedHeaders=host, Signature=sig", "secret")
	f.Add("garbage format", "key")
	f.Add("", "")
	f.Add("AWS4-HMAC-SHA256 Credential=AKID/bad/cred/format, SignedHeaders=host, Signature=sig", "key")
